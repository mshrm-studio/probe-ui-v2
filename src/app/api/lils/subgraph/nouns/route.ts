...
import { isLilFromSubgraphList } from '@/utils/dto/Lil/FromSubgraph';

const client = createClient({
    url: `${process.env.SUBGRAPH_BASE_URL}/${process.env.SUBGRAPH_API_KEY}/subgraphs/id/${process.env.LILS_SUBGRAPH_ID}`,
...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        if (!isLilFromSubgraphList(result.data.nouns)) {
            return NextResponse.json(
                { error: 'Invalid data' },
                { status: 500 }
            );
